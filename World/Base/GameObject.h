...
#define GAMECLASS(OBJ, ID) public: constexpr static int GObjID = ID;\
                                static std::weak_ptr<CGameObject> InstanceGObj(const FVector2& Coord);\
                                private: const static bool IsRegister;

//여기에 인스턴싱 내용을 넣기 / 를 위해서 챕터에 객채별 인스턴싱 템플릿 만들기
#define REGISTER_GAMECLASS(GAMEOBJ, NAME, TYPE) std::weak_ptr<CGameObject> GAMEOBJ::InstanceGObj(const FVector2& Coord) {\
